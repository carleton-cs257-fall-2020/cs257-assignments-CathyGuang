...
try:
    ...
    query = 'SELECT races.id, races.year, races.round, races.circuitID, races.name, races.date, races.url, circuits.id, circuits.name, circuits.location, circuits.country, circuits.url FROM races, circuits WHERE races.circuitID = circuits.id'
...
# Ignoring all the comments below---For later use
...
#     query = 'SELECT drivers.id, drivers.forename, drivers.surname, drivers.dob, drivers.nationality, drivers.url FROM drivers'
...
#     query = 'SELECT drivers.id, drivers.forename, drivers.surname, results.driverId, results.points FROM drivers, results WHERE drivers.id = results.driverId'
...
# for row in cursor_driver_with_results:
# 	drivers_with_results_list.append(row)

# @api.route('/topdriver?top={number}')
# def get_country_races(number):
#     driver_points_dict = {}
#     for driver in driver_list:
#         driver_points = 0
#         driver_name = driver_list[1] + " " + driver_list[2]
#         for row in drivers_with_results_list:
#             if row[0] == driver[0]:
#                 driver_points += row[3]
#         driver_points_dict[driver_name] = driver_points
...
#         cur_pair = sorted_driver_points_dict.pop()
